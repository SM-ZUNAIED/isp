...
/* ------------- SMS provider (generic, configurable) ------------- */
export type SmsConfig = {
  provider?: string;         // e.g. "custom", "bulksmsbd", "sslwireless"
  url: string;               // full URL template
  method?: "GET" | "POST";
  params?: Record<string, string>;   // for GET query or POST body
  headers?: Record<string, string>;
  body_type?: "json" | "form";       // POST body format, default json
  success_contains?: string;         // optional substring in response to treat as success
};
...
export async function sendSms(
  ...
  const fill = (v: string) =>
    v.replaceAll("{mobile}", mobile).replaceAll("{message}", encodeURIComponent(message)).replaceAll("{message_raw}", message);
  const method = (cfg.method ?? "GET").toUpperCase();

  try {
    let url = fill(cfg.url);
    let init: RequestInit = { method, headers: cfg.headers ?? {} };

    if (method === "GET") {
      const qs = new URLSearchParams();
      Object.entries(cfg.params ?? {}).forEach(([k, v]) => qs.append(k, fill(v).replace(/^%|%$/g, "")));
      // If url has no query, append; otherwise merge
      const sep = url.includes("?") ? "&" : "?";
      if (qs.toString()) url = `${url}${sep}${qs.toString()}`;
    } else {
      const filled: Record<string, string> = {};
      Object.entries(cfg.params ?? {}).forEach(([k, v]) => (filled[k] = fill(v).replace(/^%|%$/g, "")));
      if ((cfg.body_type ?? "json") === "form") {
        const form = new URLSearchParams(filled);
